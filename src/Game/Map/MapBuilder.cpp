...
MapBuilder::MapBuilder(){
	...
	senzu = ofColor(0,0,255);
	bigDotC = ofColor(167, 0, 150);
	senzuSheet.load("images/senzubean.png");
    ...
    for (int i = 0; i < mapImage.getWidth(); i++) {
        for (int j = 0; j < mapImage.getHeight(); j++) {
            ...
            if(currentPixel == boundBoundBlock){
               ...
               int randomNum = rand() % 100; // gives the variable a random number from 1 to a 100 each time a dot is added to the map -EAHC
					if( randomNum == 1){ // if the random number is 1, that dot will be replaced by a strawberry -EAHC
					...
					else if( randomNum == 2 ){ // if the random number is 2, that dot will be replaced by an Apple -EAHC
						Apple* apple = new Apple(xPos,yPos,pixelMultiplier,pixelMultiplier, pacmanSpriteSheet);
						mapInCreation->addEntity(apple);
					} else if( randomNum == 3){ // if the random number is 3, that dot will be replaced by a Cherry -EAHC
			...
			/* When there is blue pixel in the map, it will create a senzuBean */
			else if ( currentPixel == senzu){
				SenzuBean* senzuBean = new SenzuBean(xPos,yPos,pixelMultiplier,pixelMultiplier, senzuSheet);
				mapInCreation->addEntity(senzuBean);
			}
        }
