...
MenuState::MenuState() {
	...
	mapSelectButton = new Button(ofGetWidth()/2-32, ofGetHeight()/2, 64, 150, "Select Map");
...
void MenuState::tick() {
	...
	else if(mapSelectButton->wasPressed()){
		setNextState("MapSelect");
		setFinished(true);
...
void MenuState::mousePressed(int x, int y, int button){
	...
	mapSelectButton->mousePressed(x, y);
}
...
void MenuState::reset(){
	...
	setNextState("");
	startButton->reset();
	mapSelectButton->reset();
}

MenuState::~MenuState(){
	delete startButton;
	delete mapSelectButton;
	delete anim;
}